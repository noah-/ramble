...
class RambleGossipCluster {
  ...
  private void run() throws InterruptedException, IOException, URISyntaxException, NoSuchAlgorithmException, KeyServiceException {
  ...
  private List<GossipService> createCluster() throws InterruptedException, IOException, URISyntaxException {
    ...
    for (URI node : this.nodes) {
      clients.add(GossipServiceFactory.buildGossipService(node, startupMembers, null, null));
    }
    return clients;
  }
